...
def Crank_Nicolson_FVM_TVD_advection_diffusion_reaction(C0, z_cell, z_face, dz, NJ, NK, NN, dt, N_sub, sub_cells, gamma):
    ...
    for n in range(0, NN):
        # Simple progress bar
        Nsteps = max(1, int(NN/50) )
        if n % Nsteps == 0:
            if n == 0:
                ETA = '-'
            else:
                ETA = int((time.time() - starttime) * ((NN / n) - 1))
            print(f'Running simulation with {Nclasses:>3} classes... | {"#" * int(n / (NN/50)):<50} | ETA {ETA} seconds      ', end = '\r')
            sys.stdout.flush()
...
parser.add_argument('--dt', dest = 'dt', type = int, default = 300, help = 'Timestep')
...
Hs = 1 # Dummy value, not used in Case 1
...
for diffusivity_profile, label in zip((K_A, K_B), ('A', 'B')):
    ...
    pdf = lambda v: np.exp(-0.5*((v - mean_speed)/std_dev_speed)**2) / (std_dev_speed*np.sqrt(2*np.pi))
    # bin edges
    speed_class_edges = np.linspace(mean_speed - 2*std_dev_speed, mean_speed + 2*std_dev_speed, Nclasses + 1)
    fractionator = Fractionator(speed_class_edges, rho)
    # bin midpoints, used to represent the bin
    speeds = np.array([(speed_class_edges[i+1] + speed_class_edges[i])/2 for i in range(Nclasses)])
    # Fractions
    mass_fractions = np.zeros(Nclasses)
    # Parameter for romberg integration
    Nsub = 2**8 + 1
    for j in range(Nclasses):
        evaluation_points = np.linspace(speed_class_edges[j], speed_class_edges[j+1], Nsub)
    ...
    np.save(f'../data/Case1_K_{label}_block_Nclasses={Nclasses}_NJ={NJ}_dt={dt}.npy', c)
