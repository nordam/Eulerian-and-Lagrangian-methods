...
sub_cells = np.where((z_cell >= 2) & (z_cell <= 3.2))[0]
...
# Entrainment rate
Q = entrainmentrate(windspeed, Tp, Hs, rho, ift)
gamma = Q
...
for diffusivity_profile, label in zip((K_A, K_B), ('A', 'B')):
    ...
    np.save(f'../data/Case3_K_{label}_block_Nclasses={Nclasses}_NJ={NJ}_dt={dt}.npy', c)
