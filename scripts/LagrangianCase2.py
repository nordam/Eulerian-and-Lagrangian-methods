...
parser.add_argument('--dt', dest = 'dt', type = int, default = 30, help = 'Timestep [s]')
parser.add_argument('--Np', dest = 'Np', type = int, default = 1000, help = 'Number of particles')
parser.add_argument('--Nruns', dest = 'Nruns', type = int, default = 100, help = 'Number of runs')
...
for i in trange(Nruns):
    ...
    for diffusivityprofile, label in zip( (K_A, K_B), ('A', 'B')):
        ...
        # Run simulation
        H, M, Z = experiment(Z0, V0, Tmax, dt, bins, Zmax, diffusivityprofile, surfacing=False, settling=True)

        # Save output
        np.save(f'../../data/Case2_K_{label}_Lagrangian_concentration_Np={Np}_dt={dt}_{i:04}.npy', H)
        np.save(f'../../data/Case2_K_{label}_Lagrangian_moment_Np={Np}_dt={dt}_{i:04}.npy', M)
        np.save(f'../../data/Case2_K_{label}_Lagrangian_positions_Np={Np}_dt={dt}_{i:04}.npy', Z)
